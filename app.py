# =========================================================
# MSME REAL-TIME SUPPLY–DEMAND CONTROL TOWER (FINAL SAAS)
# =========================================================
...
def send_alert(msg):
    send_whatsapp(msg)
    st.sidebar.success("WhatsApp alert sent 📲")

# ---------- DATABASE ----------
conn = sqlite3.connect("msme.db", check_same_thread=False)
c = conn.cursor()
...
c.execute("INSERT OR IGNORE INTO users VALUES ('planner','plan123','Planner')")
c.execute("INSERT OR IGNORE INTO users VALUES ('viewer','view123','Viewer')")
conn.commit()

# ---------- LOGIN ----------
def login():
    st.title("🔐 MSME Control Tower Login")
...
def actions(inv_sim, risks):
    ...
    return acts
st.title("🏭 MSME Real-Time Control Tower")
...
# ================= DATA ENTRY =================
    ...
    with tab1:
        ...
        if st.button("Add Order"):
            c.execute("INSERT INTO orders VALUES (?,?,?)",(d,item,qty)); conn.commit(); st.success("Added")

    with tab2:
        item=st.text_input("Item "); on=st.number_input("On hand"); wip=st.number_input("WIP"); ss=st.number_input("Safety")
        if st.button("Add Inventory"):
            c.execute("INSERT INTO inventory VALUES (?,?,?,?)",(item,on,wip,ss)); conn.commit(); st.success("Added")

    with tab3:
        item=st.text_input("Supplier Item"); lead=st.number_input("Lead"); moq=st.number_input("MOQ")
        if st.button("Add Supplier"):
            c.execute("INSERT INTO suppliers VALUES (?,?,?)",(item,lead,moq)); conn.commit(); st.success("Added")

    with tab4:
        d=st.date_input("Capacity Date"); cap=st.number_input("Max Units")
        if st.button("Add Capacity"):
            c.execute("INSERT INTO capacity VALUES (?,?)",(d,cap)); conn.commit(); st.success("Added")
